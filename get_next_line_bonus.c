...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/23 11:25:44 by lgoncalv          #+#    #+#             */
/*   Updated: 2021/06/28 09:49:16 by lgoncalv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	if (!buffer)
		return (-1);
	size = 1;
	while (size > 0 && !ft_find_char(temp[fd], '\n'))
	{
		...
		buffer[size] = '\0';
		temp[fd] = add_to_temp(temp[fd], buffer);
	}
	free(buffer);
	*line = create_line(temp[fd]);
	temp[fd] = trim_temp(temp[fd]);
	if (size == 0)
		return (0);
	return (1);
}
